/**
 * Shared configuration loader for the CodeBuddy OpenViking memory plugin.
 *
...
export function loadConfig() {
  ...
  // apiKey: env > cliFile.api_key > codebuddy.apiKey > server.root_api_key
  ...
  // account: env > cliFile.account > codebuddy.accountId > ""
  const account =
    str(process.env.OPENVIKING_ACCOUNT, null) ||
    str(cliFile.account, null) ||
    str(cx.accountId, "");

  // user: env > cliFile.user > codebuddy.userId > ""
  const user =
  ...
  const defaultLogPath = join(homedir(), ".openviking", "logs", "codebuddy-hooks.log");
