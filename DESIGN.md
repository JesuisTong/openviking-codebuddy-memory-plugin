...
- **OV session** — `viking://session/cb-<codex-session-id>`. New captures
  derive the OV session id from the codex `session_id` with a `cb-` prefix,
...
same `cb-<codex-session-id>` OV session id for the post-compact half.
...
re-derives the same `cb-<codex-session-id>` OV session id and starts
...
```json
{
  ...
  "ovSessionId": "cb-0193af...-or-null", // null means "committed, awaiting next Stop"
...
2. Otherwise list `viking://session/cb-<codex-session-id>/history/archive_*/`
